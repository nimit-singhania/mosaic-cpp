...
The dataset `istella22.tar.gz` can be downloaded from the [data-set website](https://istella.ai/datasets/istella22-dataset/). We have provided a `istella22_data_generation.sh` file to help create train and test sets. A utility `infer` is available to assess the model performance on test data. To run the evaluation:
...
[1] [Rajeev Alur, Nimit Singhania. Precise Piecewise Affine Models from Input Output Data. EMSOFT 2014](https://drive.google.com/file/d/1ePq-5Fk-KRFPltFqEITM5hmbNSDB8EX1/view).

[2] [Domenico Dato, Sean MacAvaney, Franco Maria Nardini, Raffaele Perego, and Nicola Tonollotto. The Istella22 Dataset: Bridging Traditional and Neural Learning to Rank Evalaution. SIGIR 2022](https://dl.acm.org/doi/pdf/10.1145/3477495.3531740).
