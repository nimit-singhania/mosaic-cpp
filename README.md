...
* `istella22_v1.txt`: this consists of two features (features 125 and 140). We also created a sub-sampled version `istella_v1small.txt`.
...
These data-sets were created to assess the ability to model them using piecewise affine models. We faced multiple challenges:
* The split function that is run when a counter-example group is inconsistent with a new counter-example relies on heuristics, which often returned infeasible splits. We iterated upon the present heuristic and added another simple univariate and bivariate heuristic to improve the method operation.
* The genPredicate method that constructs a linear predicate that is true on all positive points in a group and false on negative group points returns incorrect predicate. The checking for generated predicate helps catch such bugs. The linear constraints solver seems to get stuck in local minima, and the solver was updated to use scaling to improve search efficiency. We further add direct solutions for simple predicates of the form x >= n (univariate) and x + y >= n (bivariate). These help scale the synthesis of predicates.
...
```
    make
    ./infer <model_file> <test_data.txt>
```
